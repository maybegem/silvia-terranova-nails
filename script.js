...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. IntersectionObserver per Riproduzione Automatica Video stile GIF / Reel
  ...
  if ('IntersectionObserver' in window) {
    const videoObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
          video.play().catch(() => {
            // Autoplay ignorato se il browser è in risparmio energetico
          });
        } else {
          video.pause();
        }
    ...
    gifVideos.forEach(v => videoObserver.observe(v));
  }

  // 3. Filtro Categorie Gallery (Foto & Video)
  ...
  // 4. Modal Lightbox per Ingrandire Foto o Video
        ...
        if (type === 'video') {
          ...
          modalBody.innerHTML = `
            <div style="position: relative; background: #000; border-radius: 16px; overflow: hidden;">
              <video controls autoplay loop playsinline preload="auto" style="width: 100%; max-height: 75vh; background: #000; display: block;">
                <source src="${videoSrc}" type="video/mp4">
                <source src="${videoSrc.replace('.mp4', '.mov')}" type="video/quicktime">
              </video>
              <div style="padding: 20px; text-align: center; background: #FFF;">
                <span style="font-size: 0.8rem; text-transform: uppercase; color: #B87D70; font-weight: 700;">🎥 Reel Video / ${category}</span>
